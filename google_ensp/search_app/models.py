...
class EngineModel():
    def __init__(self, model_folder="./search_app/ml_models/sauvegarde/"):
        ...
        self.categories = ["REMEMBERING", "UNDERSTANDING", "APPLYING", "ANALYZING", "EVALUATING", "CREATING"]
    ...
    def search(self, query, count=70, local=True):
        ...
        return (qr, dt), (qr_pb, dt_pb), self.categories
    ...
    def _classify_base_on_bloom(self, query_clean, data_clean, column="text_clean"):
        ...
        query_prob = (self.lr.predict_proba(query_vect) +
                      self.xgb.predict_proba(query_vect))/2
        query_prob = pd.DataFrame(query_prob, columns=self.categories)

        data_vect = self.vectoriser.transform(data_clean[column].values)
        data_prob = (self.lr.predict_proba(data_vect) +
                     self.xgb.predict_proba(data_vect))/2
        data_prob = pd.DataFrame(
            np.array(data_prob.reshape(-1, 6)), columns=self.categories)
    ...
    def _fetch_exemples(self, path="./search_app/ml_models/exemple/queries.txt", save_to="./search_app/ml_models/exemple/", count=70):
        ...
        for i, question in enumerate(questions):
            ...
            data.to_csv(save_to+"query{}.csv".format(i),
    ...
    def _fetch_data(self, query=None, count=70):
        ...
        self.response = requests.get(
            self.base_url+"search", headers=self.headers, params=self.params)
        self.data = pd.DataFrame(self.response.json()["organic"])
        return query, self.data[self.columns].copy()

    # google_ensp\search_app\ml_models\exemple\exemple0.csv
